...
def read_data(paramsInput_filePath, dataset_filePath):
    ...
    if paramsInput['load_diagram_exists'] == 1:
        ...
        dataset = verify_1min_resolution(dataset, fix_spikes=False)
    else:
...
def verify_1min_resolution(dataset, fix_spikes=False):
    df = dataset.copy()
    ...
    df = _template.merge(df, how='left', on='timestamp').fillna(0)

    if fix_spikes:
        _df = df.loc[df['load'] > 250]
        _threshold = 1.25 * _df['load'].median()
        _df = _df.loc[_df['load'] < _threshold]
